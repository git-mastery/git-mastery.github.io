...
{% set cli %}
* **To delete recent `n` commits and discard those changes entirely, do a `hard` reset** to the commit `HEAD~n`, e.g.,
  ```bash
...
* **To undo recent `n` commits, but keep changes staged, do a `soft` reset** to the commit `HEAD~n` e.g.,
  ...
  ```
* **To undo recent `n` commits, and move changes to the working directory, do a `mixed` reset** to the commit `HEAD~n`, e.g.,
  ```bash
