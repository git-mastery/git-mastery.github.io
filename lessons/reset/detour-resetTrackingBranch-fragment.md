...
```
{% set a %}
{{ show_commit('C4', desc=show_ref('main') + show_head() + show_ref('origin/main')) }}
{{ show_commit('C3') }}
{{ show_commit('C2') }}
{{ show_commit('C1', edge='') }}
{% endset %}
{% set b %}{% endset %}
{% set c %}
{{ show_commit('C4', desc=show_ref('origin/main')) }}
{{ show_commit('C3') }}
...
{{ show_commit('  ', style='light') }}
{{ show_commit('C2', desc=show_ref('main') + show_head() + show_ref('origin/main')) }}
{{ show_commit('C1', edge='') }}
{% endset %}
{{ show_transformation_columns(a, b, c) }}
<p/>

The `git update-ref refs/remotes/origin/main HEAD` commands resets the remote-tracking branch ref `origin/main` to follow the current `HEAD`.
