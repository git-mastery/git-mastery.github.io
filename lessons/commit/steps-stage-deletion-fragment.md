...
**If you’ve already deleted the file manually** (for example, using `rm` or deleting it in your file explorer), you can still stage the deletion using the `stage` command (or its synonym `add`). Even though the file no longer exists, staging the deletion records its deletion into the staging area.
