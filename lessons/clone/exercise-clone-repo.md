...
1. Clone the fork to your computer, placing it in the folder `clone-repo/shapes` (note how the target folder is `shapes`, not `gm-shapes`).<br>
   {{ icon_tip }} If you did this step correctly, you should now have a local repo named `shapes`.
1. Set up a remote named `upstream` in that `shapes` repo that points to the upstream repo [git-mastery/gm-shapes](https://github.com/git-mastery/gm-shapes).
</div>
