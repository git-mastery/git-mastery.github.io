...
<span id="outcomes">Can control what changes go into a commit.</span>
...
{{ hp_number("1") }} **Make several changes to some tracked files.** Change multiple files. Also change multiple locations in the same file.
...
{% endset %}
{% set sourcetree %}
To stage a hunk, click the `Stage` button above that hunk:<br>
<pic src="images/sourcetreeStageHunk.png" width="600" />
<p/>

To stage specific lines, select the lines before clicking the `Stage` button above that hunk:<br>
<pic src="images/sourcetreeStageLine.png" width="600" />
<p/>
Unstaging can be done similarly:<br>
<pic src="images/sourcetreeUnstageHunk.png" width="600" />
{% endset %}
...
{{ show_previous_link(tour=trail.fineTuningHistory, part="intro") }}{{ show_next_link(lesson=trail.fineTuningHistory.lessons.commitMessage) }}
</div>
<div id="extras">
</div>
