...
```bash
python3 .claude/skills/concepts-review/scripts/extract_lesson_sequence.py
```
...
* Do not suggest removing, flattening, or replacing `<tooltip>` or `popover` elements merely because static HTML extraction or a browser text projection omits the trigger text or hidden explanation. Verify the authored source and the rendered component semantics first.
* When a tooltip or popover contains a conceptual problem, preserve the component and revise the relevant visible trigger text or explanatory content unless the user explicitly asks for a syntax or component change.
